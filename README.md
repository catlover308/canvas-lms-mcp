...
This fork additionally ships the canonical private Cloudflare Worker at `https://canvas-mcp-cf.brycel.net/mcp`. The public Worker contract is deliberately smaller than the upstream stdio server: exactly six read-only student tools for the configured Pasadena City College account (`get_student_dashboard`, `list_courses`, `get_course_summary`, `list_assignments`, `get_assignment_details`, and `get_content_item`). It exposes no write tools or MCP resources. Canvas tokens, a static MCP access token, and the OAuth portal's owner secret are Worker secrets; the Canvas origin is a non-secret Wrangler variable. Remote clients can use OAuth 2.1 with S256 PKCE, CIMD or dynamic client registration, refresh tokens, and protected-resource discovery, or authenticate directly with `Authorization: Bearer <MCP_ACCESS_TOKEN>`. OAuth credentials are signed and self-contained, so the Worker remains stateless and uses no KV, database, or Durable Object. Neither MCP credential is sent to Canvas. `GET /health` publishes the canonical source repository, contract version, and exact tool names without testing or exposing the Canvas credential.
