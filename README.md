...
This fork additionally ships a stateless Cloudflare Worker at `/mcp`, with Pasadena City College as the default institution and College of the Canyons selectable through every tool's `institution` argument. Canvas tokens, a static MCP access token, and the OAuth portal's owner secret are Worker secrets; the two Canvas origins are non-secret Wrangler variables. The Canyons token is optional: without it, the complete Canyons tool and resource surface remains discoverable but returns an explicit dormant response. Remote clients can use OAuth 2.1 with S256 PKCE, CIMD or dynamic client registration, refresh tokens, and protected-resource discovery, or authenticate directly with `Authorization: Bearer <MCP_ACCESS_TOKEN>`. OAuth credentials are signed and self-contained, so the Worker remains stateless and uses no KV, database, or Durable Object. Neither MCP credential is sent to Canvas. Run `scripts/deploy-worker-with-secrets.zsh` for the initial deployment.
